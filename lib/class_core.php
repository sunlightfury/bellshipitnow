...
// *************************************************************************
	...
	class Core
	{
		...
		function __construct()
		{
			...
			$this->year = (get('year')) ? get('year') : date('Y');
			$this->month = (get('month')) ? get('month') : date('m');
			$this->day = (get('day')) ? get('day') : date('d');
		...
		{
			$sql = "SELECT a.id, a.order_inv,a.shipping_label, a.r_name, a.r_qnty, a.r_dest, a.r_city, a.country, a.city, a.courier, a.service_options, a.payment_status, a.pay_mode, a.created, a.status_courier FROM add_courier a WHERE a.act_status='1' ORDER BY a.id ASC";
			$row = self::$db->fetch_all($sql);
			return ($row) ? $row : 0;
		}
