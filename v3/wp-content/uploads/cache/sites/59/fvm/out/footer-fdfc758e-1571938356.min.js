...
jQuery(document).ready(function($){if($(".social-share-action").length){var options={trigger:"manual",template:'<div class="tooltip copied" role="tooltip"><div class="arrow"></div><div class="tooltip-inner"></div></div>'};$(".social-share-action .copy-clipboard").tooltip(options),$(this).find(".share_social").on("click",function(e){var share_service=$(this).data("service"),share_title=encodeURI($(this).data("text")),share_url=encodeURI($(this).data("url"));switch(share_service){case"facebook":e.preventDefault();var facebookWindow=window.open("https://www.facebook.com/sharer/sharer.php?u="+share_url,"facebook-popup","height=350,width=600");facebookWindow.focus&&facebookWindow.focus();break;case"twitter":var twitter_url="https://twitter.com/intent/tweet?text="+share_title+"&url="+share_url;window.open(twitter_url,"","left=0,top=0,width=550,height=450,personalbar=0,toolbar=0,scrollbars=0,resizable=0");break;case"email":var mailto_link="mailto:?subject="+share_title+"&body="+share_url;location.href=mailto_link;default:var clipboard=new ClipboardJS(".social-share-action .copy-clipboard");$(this).tooltip("show");var element=$(this);setTimeout(function(){$(element).tooltip("hide")},1e3),clipboard.on("success",function(e){e.clearSelection()})}})}if($(".mobile-share-action").length){var options={trigger:"manual",template:'<div class="tooltip copied" role="tooltip"><div class="arrow"></div><div class="tooltip-inner"></div></div>'};$(".mobile-share-action .copy-clipboard").tooltip(options),$(this).find(".mobile-share-action").on("click",function(e){var share_service=$(this).data("service"),share_title=encodeURI($(this).data("text")),share_url=encodeURI($(this).data("url"));switch(share_service){case"facebook":e.preventDefault();var facebookWindow=window.open("https://www.facebook.com/sharer/sharer.php?u="+share_url,"facebook-popup","height=350,width=600");facebookWindow.focus&&facebookWindow.focus();break;case"twitter":var twitter_url="https://twitter.com/intent/tweet?text="+share_title+"&url="+share_url;window.open(twitter_url,"","left=0,top=0,width=550,height=450,personalbar=0,toolbar=0,scrollbars=0,resizable=0");break;case"email":var mailto_link="mailto:?subject="+share_title+"&body="+share_url;location.href=mailto_link,console.log(mailto_link);default:var clipboard;new ClipboardJS(".mobile-share-action .copy-clipboard").on("success",function(e){$(".mobile-share-action .copy-clipboard").tooltip("show"),setTimeout(function(){$(".mobile-share-action .copy-clipboard").tooltip("hide")},2e3),e.clearSelection()})}})}});
/*!
 * clipboard.js v2.0.0
 * https://zenorocha.github.io/clipboard.js
 * 
...
window.Clipboard=function(window,document,navigator){var textArea,copy;function isOS(){return navigator.userAgent.match(/ipad|iphone/i)}function createTextArea(text){(textArea=document.createElement("textArea")).value=text,document.body.appendChild(textArea)}function selectText(){var range,selection;if(isOS()){var editable=textArea.contentEditable,readOnly=textArea.readOnly;textArea.contentEditable=!0,textArea.readOnly=!1,(range=document.createRange()).selectNodeContents(textArea),(selection=window.getSelection()).removeAllRanges(),selection.addRange(range),textArea.setSelectionRange(0,999999),textArea.contentEditable=editable,textArea.readOnly=readOnly}else textArea.select()}function copyToClipboard(el){var successful=document.execCommand("copy");document.body.removeChild(textArea);var msg=successful?"Copied to clipboard":"Whoops, not copied!",options={trigger:"manual"};el.attr("data-original-title",msg).tooltip(options),el.tooltip("show"),setTimeout(function(){el.tooltip("hide")},2e3)}return{copy:copy=function(text,el){createTextArea(text),selectText(),copyToClipboard(el)}}}(window,document,navigator),function($){$(document).ready(function(){var userAgent,ieReg,ie;$(".w-pg-gView .w-pg-share").on("click",function(e){var shareService=$(this).data("service"),shareTitle=$(this).data("text"),shareUrl=$(this).data("url");switch(shareService){case"facebook":var facebookWindow=window.open("https://www.facebook.com/sharer/sharer.php?u="+shareUrl,"facebook-popup","height=350,width=600");facebookWindow.focus&&facebookWindow.focus();break;case"twitter":var twitterURL="https://twitter.com/intent/tweet?text="+shareTitle+"&url="+shareUrl;window.open(twitterURL,"","left=0,top=0,width=550,height=450,personalbar=0,toolbar=0,scrollbars=0,resizable=0");break;case"email":var mailtoLink="mailto:?subject="+shareTitle+"&body="+shareUrl;window.location.href=mailtoLink;break;case"link":var text=$(this).attr("data-copy"),el=$(this);Clipboard.copy(text,el);break;default:e.preventDefault()}}),$.fn.slideSlick=function(){var numberThumbnail=$(this).find(".w-pg-thumbnail-item").length;$(this).slick({infinite:!1,slidesToShow:5,slidesToScroll:1,initialSlide:0,nextArrow:'<a class="slick-next"><span class="w-pg-arrow w-pg-arrow-right"></span></a>',prevArrow:'<a class="slick-prev"><span class="w-pg-arrow w-pg-arrow-left"></span></a>',responsive:[{breakpoint:767,settings:{slidesToShow:numberThumbnail>3?2.75:numberThumbnail,slidesToScroll:1,initialSlide:1}}]})},$(".w-pg-overview-mode .w-pg-box-item").on("click",function(){var widthTumbnailItems;$target=$($(this).data("pg-target")),$slideTo=$(this).data("pg-slide-to"),$target.closest(".w-pg-gView").addClass("show"),$target.carousel($slideTo),0==$target.find(".slick-slide").width()&&$target.find(".w-pg-carousel-indicators").slick("setPosition")}),$(".w-pg-carousel-indicators .w-pg-thumbnail-item").on("click",function(){$target=$(this).data("target"),$slideTo=$(this).data("slide-to"),$($target).carousel($slideTo)}),$(".carousel-control-prev, .carousel-control-next").on("click",function(){$target=$(this).attr("href"),$slideTo=$(this).data("slide"),$($target).carousel($slideTo)}),$(".w-pg-gView .close").on("click",function(){$(this).closest(".w-pg-gView").removeClass("show")}),$(".w-pg-carousel").bind("slid.bs.carousel",function(){var $pgItemActive,slideNumber=$(this).find(".carousel-inner .carousel-item.active .w-pg-item").data("slide-number");if(null!=slideNumber&&""!==slideNumber){var $thumbnailItem=$(this).find(".w-pg-carousel-indicators .slick-slide .w-pg-thumbnail-item[data-slide-to="+slideNumber+"]");$(this).find(".w-pg-carousel-indicators .slick-slide").removeClass("current");var slickItemIndex=$thumbnailItem.closest(".slick-slide").addClass("current").data("slick-index");$thumbnailItem.closest(".w-pg-carousel-indicators").slick("slickGoTo",slickItemIndex),$thumbnailItem.closest(".w-pg-carousel-indicators").slick("setPosition")}}),$(".w-pg-carousel-indicators").slideSlick(),userAgent=window.navigator.userAgent,ieRegex=/msie|Trident.*rv[ :]*11\./gi,(ie=ieRegex.test(userAgent))&&$(".so-widget-dpd-picture-gallery-widget .w-object-cover").each(function(){var $container=$(this).parent(),imgUrl=$(this).prop("src"),onlyImage=0==$(this).siblings().length;imgUrl&&(onlyImage?$container.css("backgroundImage","url("+imgUrl+")").addClass("w-custom-object-cover"):$(this).wrap($("<div/>").addClass("w-for-custom-object-cover").addClass("w-custom-object-cover").css({height:$(this).height(),width:"100%",backgroundImage:"url("+imgUrl+")"})))}),$(document).on("click",".download-picture-047",function(){var data=$(this).data(),xhr=new XMLHttpRequest;xhr.open("GET",data.url),xhr.responseType="blob",xhr.onload=function(){var _click=function(node){try{node.dispatchEvent(new MouseEvent("click"))}catch(e){var evt=document.createEvent("MouseEvents");evt.initMouseEvent("click",!0,!0,window,0,0,0,80,20,!1,!1,!1,!1,0,null),node.dispatchEvent(evt)}},_bom=function(blob,opts){return void 0===opts?opts={autoBom:!1}:"object"!=typeof opts&&(opts={autoBom:!opts}),opts.autoBom&&/^\s*(?:text\/\S*|application\/xml|\S*\/\S*\+xml)\s*;.*charset\s*=\s*utf-8/i.test(blob.type)?new Blob([String.fromCharCode(65279),blob],{type:blob.type}):blob},_saveAs=function(blob,name,opts){if("msSaveOrOpenBlob"in navigator){var a;if(name=name||blob.name||"download","string"==typeof blob)(a=document.createElement("a")).href=blob,a.target="_blank",setTimeout(function(){_click(a)});else navigator.msSaveOrOpenBlob(_bom(blob,opts),name)}else{var URL=window.URL,a=document.createElement("a");name=name||blob.name||"download",a.download=name,a.rel="noopener","string"==typeof blob?(a.href=blob,a.origin!==location.origin?_click(a,a.target="_blank"):_click(a)):(a.href=URL.createObjectURL(blob),setTimeout(function(){URL.revokeObjectURL(a.href)},4e4),setTimeout(function(){_click(a)},0))}}(xhr.response,data.name,data.opts)},xhr.onerror=function(){window.open(data.url,data.name)},xhr.send()})})}(jQuery);
