...
<div id="page-wrap">
        ...
        <table id="meta">
            <tr>
				...
				<table id="items">
					...
					<?php echo $row->r_dest." | ".$row->c_address; ?> </br>
    ...
    <table id="items">
        <tr>
			...
			<th align="right"><p style="color:white;">Add Weight</p></th>
		...
		$sumador_total=0;
		$net=0;$iten=0;
		$sql = "SELECT COUNT(*) total FROM cons_products";
		$result = $db->query($sql);
		$fila = $result->fetch_assoc();
		$iten=$iten+$fila['total'];	
		$net=$iten;
		$det_weight=0;
		$sql = $db->query("SELECT * FROM consolidate, cons_products WHERE consolidate.idcon=cons_products.idcon AND consolidate.id='".Filter::$id."'");

		while ($row=mysqli_fetch_array($sql))
			{
			$id=$row["id"];
			$detail_weight=$row['r_weights'];
			$det_weight=$det_weight+$row['r_weights'];
			$detail_add_pound=$row['c_add_pound'];
			$detail_qty=$row['r_qntys'];
			$description=$row['r_descriptions'];
			$reexpe=$row['reexpedition'];
			$t_safe=$row['r_costtotals'];
			$t_total=$row['r_costtotal'];
			$track=$row['order_inv'];
			...
			$precio_add_pound=$precio_venta_radd*$detail_add_pound;
		...
		?>
		<tr class="item-row">
            <td class="<?php echo $clase;?>"><?php echo $description; ?></td>
            ...
            <td class="<?php echo $clase;?>" align="right"><?php echo $core->c_value_pound; ?></td>
        </tr>
		<?php 
	...
	?>
		...
		<tr>
			...
			<td align="right" class="total-value balance"><div class="due"> <p style="color:white;"><b><?php echo $core->currency;?> <?php echo $t_total; ?> </b></p></div></td>
		</tr>
    </table>
